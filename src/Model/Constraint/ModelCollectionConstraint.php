...
final class ModelCollectionConstraint implements ConstraintInterface
{
    ...
    public function validate(string $path, $input, ValidatorInterface $validator = null): array
    {
        ...
        if ($this->recursive) {
            ...
            foreach ($input->getModels() as $i => $model) {
    ...
    /**
     * @param ModelCollectionInterface $input
     */
    private function validInputTypeOrException($input)
    {
        if (!$input instanceof ModelCollectionInterface) {
            throw new \RuntimeException(
                sprintf('Invalid input, instance of %s needed', ModelCollectionInterface::class)
            );
        }
    }
}
